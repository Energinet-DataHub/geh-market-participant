...
public sealed class ApimCertificateStoreHealthCheck : IHealthCheck
{
    ...
    public ApimCertificateStoreHealthCheck(IApimCertificateStore apimCertificateStore)
    {
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        try
        {
            await _apimCertificateStore.GetCertificateIdentifiersAsync().ConfigureAwait(false);
...
#pragma warning restore CA1031
        {
            return HealthCheckResult.Unhealthy("Could not connect to APIM.");
        }
    }
}
