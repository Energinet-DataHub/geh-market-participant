...
[UnitTest]
public sealed class ActorStatusChangedEventDispatcherTests
{
    [Fact]
    public async Task TryDispatchAsync_integrationEventNull_ThrowsException()
    {
        ...
        var eventParser = new ActorStatusChangedIntegrationEventParser();
        var sut = new ActorStatusChangedEventDispatcher(
            eventParser,
    ...
    public async Task TryDispatchAsync_SendingCorrectEvent_ReturnsTrue()
    {
        ...
        var actualEvent =
            eventParser.Parse(message.Body.ToArray()) as
                Integration.Model.Dtos.ActorStatusChangedIntegrationEvent;
