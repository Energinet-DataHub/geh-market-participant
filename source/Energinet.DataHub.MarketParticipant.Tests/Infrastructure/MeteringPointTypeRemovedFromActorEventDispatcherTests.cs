...
[UnitTest]
public sealed class MeteringPointTypeRemovedFromActorEventDispatcherTests
{
    [Fact]
    public async Task TryDispatchAsync_integrationEventNull_ThrowsException()
    {
        ...
        var meteringPointTypeEventParser = new MeteringPointTypeRemovedFromActorIntegrationEventParser();
        var sut = new MeteringPointTypeRemovedFromActorEventDispatcher(
    ...
    public async Task TryDispatchAsync_SendingCorrectEvent_ReturnsTrue()
    {
        ...
        var actualEvent =
            eventParser.Parse(message.Body.ToArray()) as
                Integration.Model.Dtos.MeteringPointTypeRemovedFromActorIntegrationEvent;
