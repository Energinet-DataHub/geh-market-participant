...
{
    [UnitTest]
    public class ActorNameChangedIntegrationEventParserTests
    {
        [Fact]
        public void Parse_InputValid_ParsesCorrectly()
        {
            // arrange
            var target = new ActorNameChangedIntegrationEventParser();
            ...
            var actualEvent = ActorNameChangedIntegrationEventParser.Parse(actualBytes);
        ...
        public void Parse_InvalidGuid_ThrowsException()
        {
            ...
            var contract = new ActorNameChangedIntegrationEventContract
            {
                ...
                Type = nameof(ActorNameChangedIntegrationEvent),
                EventCreated = Timestamp.FromDateTime(DateTime.UtcNow)
            };

            // Act + Assert
            Assert.Throws<MarketParticipantException>(() => ActorNameChangedIntegrationEventParser.Parse(contract.ToByteArray()));
        }
        ...
        public void Parse_InvalidInput_ThrowsException()
        {
            ...
            // Act + Assert
            Assert.Throws<MarketParticipantException>(() => ActorNameChangedIntegrationEventParser.Parse(new byte[] { 1, 2, 3 }));
        }
    }
}
